...
import { AgendaActorNotFoundError, getAgendaWeek } from '@/server/agenda'
import { withEndpointObservability } from '@/lib/observability'

export async function GET(request: Request) {
  return withEndpointObservability({ endpoint: 'api.agenda.week', method: 'GET' }, async () => {
    ...
    try {
      ...
      const agendaWeek = await getAgendaWeek(prisma, {
      ...
      return NextResponse.json(agendaWeek)
    } catch (error: unknown) {
