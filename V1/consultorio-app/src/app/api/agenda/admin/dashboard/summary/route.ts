...
import { AuditAction, Prisma, type AppointmentStatus } from '@prisma/client'
...
function buildSetupChecklist(input: {
  ...
  const items: SetupChecklistItem[] = [
    ...
    {
      id: 'public_profile',
      label: 'Perfil público para agendado',
      done: Boolean(profile?.slug?.trim()),
      hint: 'Define tu URL personalizada para aparecer en el directorio de pacientes.',
      actionLabel: 'Configurar URL',
      actionHref: '/medico/configuracion?tab=perfil',
    },
    {
      id: 'profile_branding',
...
export async function GET() {
  return withEndpointObservability(
    { endpoint: 'api.agenda.dashboard.summary', method: 'GET' },
    async () => {
      try {
