...
**Remediacion de la auditoria (2026-09-03/04).** Una revision completa del repositorio encontro catorce hallazgos; el plan de correccion vive en `docs/superpowers/plans/2026-09-03-remediacion-auditoria.md`. Entregados: **A** (el cron de produccion llamaba a rutas inexistentes, asi que la cola de notificaciones no corria ni se purgaba el buzon; ademas TTL de preconsultas), **B** (certificado de firma y contrasenas fuera del repositorio), **C** (compuerta de comandos por rol, rebanada 2 del paso 27), **D** (CSP del webview, contrato Zod en la frontera IPC, mock fuera del instalador, lint), **E** (integracion continua del escritorio y suites separadas) y **H** (documentacion al dia). **F** quedo decidido (la IA de texto sale por el portal) y se implementa con el paso 16. De **G** se entrego lo que protege algo —la guarda contra la deriva del cuestionario de antecedentes y la clasificacion de dato de los 51 modelos, con prueba que la exige— y **se difiere la reorganizacion de archivos**: partir `public-booking-service.ts` o mover los 80 archivos planos de `desktop-app/src/` choca de frente con cada rama abierta (el paquete A ya edita ese servicio; el paso 27 tiene dos rebanadas por venir sobre el escritorio). Un renombrado masivo no se auto-mezcla con ediciones de contenido: conviene hacerlo cuando `dev` haya absorbido las ramas en vuelo y antes de abrir las siguientes.

### Hallazgo de la clasificacion (2026-09-05)

Clasificar los modelos saco a la luz lo que la regla 4.4 existe para evitar: el portal **todavia persiste expediente clinico en PostgreSQL**. `ClinicalRecord`, `Encounter`, `ClinicalNote`, `ClinicalNoteVersion`, `Prescription`, `PrescriptionItem`, `PatientInstruction`, `ClinicalDocument` y los tres modelos dentales no son codigo muerto: `encounter-service.ts` los escribe y hay tres rutas de administracion montadas que lo llaman. Llegaron en el commit inicial de V2 (2026-06-09, el mismo dia del giro local-first) y nadie los toco desde entonces: son el portal **anterior** a la decision de arquitectura, que sobrevivio al pivote.

Contradice la residencia 1 (`01_contexto_v2.md`) y la regla 4.1. Retirarlo toca datos y rutas vivas —hay que contar filas antes de borrar tablas— asi que queda como decision del dueño, con su propia rama. Los modelos estan anotados como vestigio en el esquema para que nadie construya encima.
...
Integracion continua y suites (2026-09-04, paquete E de la misma remediacion):

- **CI de la app del medico** (`.github/workflows/desktop-verify.yml`). Hasta ahora solo se verificaba el portal; el escritorio se probaba a mano. Dos trabajos en cada PR y push a `dev`/`main` que toque `V2/desktop-app/`: frontend (lint, tipos, pruebas y build) y Rust (`cargo test` y `cargo clippy --lib -- -D warnings`), ambos sobre el build **por defecto**, sin los features nativos que exigen CMake y LLVM. No construye el instalador: eso sigue siendo el proceso del paso 9. El `.cargo/config.toml` con las rutas de OpenSSL no se versiona, asi que Linux resuelve con `libssl-dev` del sistema.
- **Suites separadas en el portal.** `vitest.unit.config.ts` (paralelo) y `vitest.integration.config.ts` (secuencial, base compartida) extienden la configuracion base; `npm test` corre ambas en orden y `test:unit`/`test:integration` quedan disponibles por separado. Revierte a paralelo lo que en junio de 2026 hubo que serializar entero por carreras sobre la base compartida: la razon solo aplicaba a integracion. Las unitarias bajan a ~2.6 s.
- **Hallazgo:** el workflow de despliegue nunca aplicaba migraciones antes de correr la suite de integracion, asi que esas pruebas corrian contra una base vacia. Se agrega `prisma migrate deploy` antes de las pruebas y los pasos pasan a llamarse por lo que son (unitarias, integracion, extremo a extremo).

Pendiente de este paquete: 12 advertencias de clippy en codigo de prueba (agrupacion de digitos, un cast redundante) impiden exigir `--all-targets`; hoy la compuerta cubre la biblioteca, que es lo que se distribuye.
...
## Paso 27 - Estaciones y separacion de roles (recepcion / clinica)

Plan detallado: `14_plan_estaciones_y_roles.md` (21 puntos, sin decisiones abiertas).

| Campo | Definicion |
|---|---|
| Objetivo | Permitir que una recepcionista opere recepcion y caja sin acceso al expediente clinico, en la misma maquina del medico o en una propia, sin romper la promesa local-first ni meter contenido clinico legible en la nube. |
| Decision revertida | `12_inventario_funcional_v1.md:57` difirio multi-usuario ("multi-usuario local-first es complejo; un dispositivo por consultorio en MVP"). El argumento sigue siendo valido; lo que cambio es que ahora esta medido: el bloque OPERATIVO resulto casi autocontenido (`visits.patient_id`, `visits.encounter_id`, `payments.patient_id` son referencias suaves sin FK; `operations.rs` solo une `visits ← resources`), asi que separar bases es la via barata y no la cara. Se revierte con costo conocido, no por optimismo. |
| Requisitos relacionados | RNF01 (autenticacion, **roles** y sesiones seguras), RNF02 (trazabilidad), RNF06 (privacidad y retencion), RF17, RF26, RF27, RF30, RF38; extiende el paso 10 (operacion presencial) y el paso 26 (presupuestos y saldos dentales). |
| Entrada necesaria | Paso 10 DONE (visits, caja, recursos, folios) y paso 26 rebanada 3 DONE (presupuestos con saldos y abonos por caja). Para la rebanada 4, contrato de sincronizacion del paso 6 en produccion. |
| Skills IA recomendadas | `superpowers:writing-plans`, `superpowers:test-driven-development`, `superpowers:verification-before-completion`, `impeccable` |
| Se construye | (1) Frontera de datos sellada entre CLINICO, CONTACTO, FACTURABLE y OPERATIVO dentro de una base; (2) identidad de usuario con envoltura de llave por persona, compuerta de comandos y actor en bitacora; (3) dos bases fisicas con llaves distintas, donde la estacion de recepcion carece del archivo clinico; (4) relevo cifrado extremo-a-extremo entre dos equipos, reusando el patron del buzon. |
| Se valida con | Con una maquina: el medico entrega un PIN que abre solo Recepcion y caja, y ninguna ruta llega al expediente. Con dos maquinas: la recepcionista registra llegadas y cobra en la suya, el medico atiende y tambien cobra en la suya, y ambas convergen sin que ninguna tenga que estar encendida al mismo tiempo. |
| Compuerta de avance | Copiar el `operativo.db` de la estacion de recepcion a otra maquina y comprobar que lo unico de origen clinico dentro son los conceptos FACTURABLE que el medico libero — ni notas, ni diagnosticos, ni odontograma, ni antecedentes. Ademas: los dos cajones cuadran por separado y consolidados, y ningun cobro se pierde ni se duplica al fusionar. |
| Push recomendado | Por rebanada cerrada, ramas `v2/paso27-<rebanada>` hacia `dev`. La rebanada 1 se puede entregar sola: no cambia comportamiento visible y deja la frontera protegida por prueba. |

Clasificacion de datos: este paso introduce una clase nueva. **FACTURABLE** = descripcion del procedimiento cobrado, redactada o confirmada por el medico, residente en la estacion operativa e impresa en el recibo que se entrega al paciente. Es el minimo necesario para cobrar y comprobar; no cruzan diagnostico, hallazgos, evolucion, antecedentes ni nota. Nivel de detalle ajustable por consultorio: **detallado** por defecto en `ODONTOLOGY`, **generico** por defecto en `GENERAL_MEDICINE` (un consultorio recien instalado nunca empieza filtrando de menos). Los deltas que viajan entre estaciones incluyen FACTURABLE y van sellados: la nube guarda ciphertext que no puede abrir, mismo precedente que los documentos del buzon del paso 6.

### Rebanadas verticales

**Rebanada 1 — Sellar la frontera (sin multiusuario).** Todo dentro de una sola base y un solo usuario; al terminar, la app se comporta igual que hoy. Partir `patients` en identidad (CONTACTO) y clinico. Soltar la unica arista dura que cruza la frontera (`payments.budget_id → dental_budgets`, migracion v22) invirtiendo el calculo de saldos. Separar **cobro de aplicacion**: `payments` pasa a ser hecho inmutable con folio y nace `payment_allocations` como capa mutable; el saldo a favor del paciente se deriva de la diferencia, sin tabla propia. Degradar `validate_budget_payment` de rechazo duro a aviso. Aislar `operations.rs` de toda tabla CLINICO y dejar una prueba de frontera que falle si un paso futuro las vuelve a mezclar. Concepto facturable y recibo imprimible. Estacion como entidad y folio con prefijo por estacion — aqui y no despues, para no migrar `payments` dos veces sobre datos contables con folio emitido.

**Rebanada 2 — Identidad, llaves y compuerta de comandos.** La base deja de cifrarse con la passphrase del medico: DEK aleatoria, `PRAGMA rekey`, y un `keys.json` junto al `.db` con una envoltura por persona (Argon2id). Vive fuera de la base por necesidad: no se puede leer la envoltura desde el archivo que la envoltura abre. Actor y estacion en la sesion; compuerta que niega por defecto sobre los 100 comandos Tauri, con la particularidad de que depende de argumentos y no solo de nombres (`register_payment` si es de recepcion, salvo `REFUND` contra saldo no asignado). Actor, estacion y `authorized_by` en `clinical_audit`. UI por rol, bloqueo por inactividad y cambio rapido de usuario. **Al cerrar esta rebanada, un medico solo ya puede dar acceso de recepcion en su propia maquina** — es el corte natural si se quiere entregar valor antes de las dos estaciones.

**Rebanada 3 — Dos bases fisicas.** `operativo.db` y `clinico.db` con llaves y envolturas distintas. La estacion de recepcion se instala solo con la operativa; la clinica lleva ambas, porque tambien cobra. La asimetria es en un solo sentido. Replica de CONTACTO y FACTURABLE, con la estacion clinica como autoridad. Aqui lo clinico deja de ser un permiso que se puede saltar y pasa a ser ausencia de archivo.

**Rebanada 4 — Relevo cifrado entre estaciones.** Primera rebanada que **toca el portal**: las tres anteriores son integramente `desktop-app`. Se reusa el patron de transporte del buzon (token de dispositivo, cursor, inbox, ACK, TTL, sobre sellado), pero no su topologia — el buzon es unidireccional y de un solo consumidor, y esto es bidireccional entre dos pares. Tres supuestos del contrato vigente hay que cambiarlos: "un dispositivo activo por medico" (`13_contrato_sincronizacion.md:19`) desconectaria al medico al dar de alta recepcion; el ACK que purga de inmediato (`:57`) borraria el evento antes de que el segundo lo lea; y el sobre que solo sella hacia el medico. Se agrega emparejamiento de estaciones con intercambio de llaves publicas (sin que el medico comparta su login del portal) y un camino de re-linea base para la estacion que pase mas de 30 dias sin sincronizar, porque sus deltas se purgaron y la nube nunca guardo un estado completo.

Invariantes que se rompen con dos cajones (el medico tambien cobra) y como se resuelven, en detalle en `14_plan_estaciones_y_roles.md` §6: caja unica pasa a unica por estacion; folio con prefijo por estacion; corte del dia por cajon mas un consolidado; sobreabono a presupuesto resuelto por construccion — los pagos no se tocan nunca y solo se recalculan las asignaciones, con una regla determinista que hace converger a ambas estaciones sin coordinarse y sin intervencion humana, dejando el excedente como saldo a favor.

Orden recomendado: 1 → 2 → 3 → 4, estrictamente. Cada rebanada depende de la anterior y no hay atajos: la 2 reencripta una base que la 1 debe haber dejado ordenada, la 3 parte un archivo que la 2 debe saber abrir con dos credenciales, y la 4 sincroniza dos archivos que la 3 debe haber creado. Dentro de la rebanada 1, la separacion cobro/aplicacion va al final y con su propio ensayo de restauracion: es la unica migracion del paso que toca datos contables con folio ya emitido.

Riesgo mayor del paso: el rekey de la rebanada 2 toca la llave de una base con expedientes reales. Backup verificado antes y `restore_drill.rs` extendido para probar restauracion post-rekey, o no se hace.

Estado: 🚧 EN PROGRESO — rebanadas 1 (2026-08-02) y 2 (2026-09-03) entregadas en `v2/paso27-frontera-datos`; restan 3 y 4. Plan cerrado y sin decisiones abiertas en `14_plan_estaciones_y_roles.md`; seis decisiones de producto fijadas ahi (dos cajones, saldo a favor, reembolso solo con autorizacion del medico, recibo con detalle del tratamiento, defaults por perfil clinico, relevo por nube).

Rebanada 1 (2026-08-02): frontera sellada, migraciones v24-v30. **Identidad separada de lo clinico** por expand/contract: v24 crea y puebla `patient_identities` (CONTACTO), los lectores se mueven, v25 suelta las columnas de `patients`. v26 clasifica al responsable/tutor como CONTACTO —`birth_date` ya vivia en la identidad, asi que recepcion ya sabe quien es menor y el contacto del tutor no agrega inferencia clinica; dejarlo del lado clinico impedia llamarlo—. **Cobro y aplicacion se parten** (v27): `payments` queda como hecho inmutable con folio y nace `payment_allocations` como capa mutable; lo no asignado *es* el saldo a favor, derivado, sin nada que sincronizar. `billable_items` (FACTURABLE) deja que la caja cobre sin leer el expediente y con eso desaparece la arista dura `payments.budget_id → dental_budgets`. v28 mete el reembolso bajo autorizacion del medico, de un solo uso y revalidando el saldo al entregar. Concepto del recibo con tres niveles y default por perfil clinico, mas recibo imprimible que aclara que no es CFDI. v29 suelta una segunda arista que aparecio al escribir la prueba (`dental_lab_orders` apuntaba a `patients`). v30 hace de la estacion una entidad con su cajon y su serie de folios, continuando la numeracion en vez de reiniciarla.
...
Rebanada 2 (2026-09-03): identidad, llaves y compuerta de comandos. **Llave por persona** (`keyring.rs`): la base se cifra con una DEK aleatoria y `keys.json`, junto al `.db`, guarda una envoltura Argon2id por persona; la credencial identifica a quien abre (no se pide usuario) y `unlock_database` devuelve el actor. El rekey escribe las envolturas antes de tocar la llave y deja respaldo pre-rekey. **Compuerta central** (`authz.rs`): tabla exhaustiva de los 112 comandos con su capacidad (`Public`, `Operations`, `Clinical`, `Admin`) y una funcion `gated` que envuelve al despachador de `generate_handler!`: por cada invocacion lee el rol de la sesion y decide **antes** de despachar; niega por defecto (comando sin politica o base bloqueada) y RECEPCION solo pasa `Operations`. `start_visit_encounter` es clinico: recepcion marca la llegada, abrir el expediente es del medico. La unica decision dependiente de argumentos —`register_payment` con `kind=REFUND`— se remata dentro del comando y queda declarada en `ARGUMENT_CHECKS`; recepcion entrega reembolsos solo via `emit_authorized_refund`. **Actor en la bitacora** (v31): `clinical_audit` gana `actor_id`, `actor_role`, `station_id` y `authorized_by`; el sello vive en una tabla TEMP de la conexion que `unlock_database` llena, asi los seis `audit()` de los modulos lo leen sin cambiar de firma y no viaja en el respaldo; el reembolso emitido registra ademas quien lo autorizo. **UI por rol y bloqueo** (`rolePolicy.ts`): recepcion abre directamente en Recepcion y caja, sin navegacion clinica ni boton Atender; bloqueo por inactividad a los 10 minutos y "Cambiar de usuario" (bloquear y abrir con otra credencial), que es tambien el camino de autorizacion presencial del reembolso.

Compuerta cubierta por la **prueba de frontera de comandos** (`authz::tests`): lee `lib.rs`, exige que cada comando registrado tenga politica (y ninguna politica sobre), recorre la tabla entera como RECEPCION comprobando que todo lo clinico y administrativo se niega y todo lo operativo pasa, y falla por inversion si un comando clinico se mueve a `Operations`. `restore_drill.rs` gana el drill post-rekey: la passphrase ya no abre la base, el respaldo se restaura con su `keys.json`, sin `keys.json` no abre, y el respaldo pre-rekey sigue abriendo con la passphrase.

Verificacion (rebanada 2): 284 pruebas de Rust en verde (+11: politica completa, negacion por defecto, base bloqueada, frontera de recepcion, comandos de la pantalla de Recepcion, medico completo, inversion, reembolso por argumento, actor en bitacora, drill post-rekey), `cargo clippy` sin advertencias nuevas, 114 pruebas de TS (+3: rol desconocido = menor privilegio, vistas por rol, bloqueo) y `tsc + vite build` ok. Pendiente para la rebanada 3: lo clinico sigue siendo un permiso —recepcion abre la misma base con su propia envoltura— y pasa a ser ausencia de archivo cuando `operativo.db` y `clinico.db` se partan.
