...
describe("AI usage metadata sync (paso 11)", () => {
  ...
  it("accepts a clinical aid report generated by the desktop app", async () => {
    const email = uniqueEmail("doctor-ai-clinical-aid");
    ...
    try {
      const account = await createDoctorAccount({
        ...
        firstName: "Mara",
        lastName: "León",
        professionalName: "Dra. Mara León",
        licenseNumber: "1234567",
        specialty: "GENERAL_MEDICINE",
        termsVersion: "2026-05",
        privacyVersion: "2026-05"
      });
      const { deviceToken } = await linkSyncDevice(account.user.id, "PC ayuda clínica");
      const device = await authenticateSyncDevice(bearerRequest(deviceToken));
      const externalRunId = randomUUID();

      const result = await recordAiUsageBatch(device, {
        runs: [
          {
            externalRunId,
            usageType: "CLINICAL_AID",
            status: "APPROVED",
            providerName: "gemini",
            providerType: "LLM",
            modelVersion: "gemini-3-flash",
            promptVersion: "clinical-aid/v1",
            estimatedCostCents: 2,
            latencyMs: 850,
            occurredAt: "2026-06-20T18:00:00.000Z",
            inputReference: {
              kind: "LOCAL_AI_RUN_INPUT",
              localRunId: externalRunId
            },
            outputReference: {
              kind: "LOCAL_AI_RUN_OUTPUT",
              localRunId: externalRunId
      ...
      expect(result.reported).toBe(1);
      const stored = await prisma.aiUsageLog.findUnique({
        where: {
      ...
      expect(stored?.usageType).toBe("OTHER");
    } finally {
      await cleanupUserByEmail(email);
    }
  });
