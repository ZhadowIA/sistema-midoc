// Mock de navegador de la app de escritorio (solo diseño/desarrollo).
//
// Vive fuera de `ipc.ts` por dos razones: el bundle de produccion no debe
// cargar ~1600 lineas de datos simulados, y asi el mock se puede probar bajo
// Node (no toca `window`) para verificar que responde lo mismo que declara el
// contrato de `ipcSchemas.ts`. `ipc.ts` lo importa de forma dinamica y solo en
// desarrollo.
...
/** Responsable/tutor del paciente (paso 18), nulo cuando es mayor de edad. */
type MockGuardian = {
  name: string;
  ...
  patients: [
    {
      ...
      // Responsable/tutor (paso 18): `PatientRecord` de Rust siempre los trae.
      guardian: null as MockGuardian,
      is_minor: false
    },
    {
      id: "pat-2",
...
/**
 * Proyecta el registro interno de ejecuciones de IA a la forma `AiRun` que
 * devuelve Rust. El mock lleva por dentro solo lo que necesita para la caja de
 * creditos; hacia la UI tiene que verse como el backend real.
 */
function mockAiRunDto(run: { id: string; usage_type: string; cost_cents: number; status: string }) {
  return {
    id: run.id,
    ...
    prompt_version: "v1",
    status: run.status,
    estimated_cost_cents: run.cost_cents,
    latency_ms: 0,
...
function mockDetail() {
  ...
  return {
    encounter: {
      ...
      // `Encounter` de Rust siempre dice de que paciente es el encuentro.
...
function matchPatientsMock(fullName: string | null | undefined, phone: string | null | undefined) {
  ...
  return mockState.patients
    ...
    .map(({ p, matched_name, matched_phone }) => ({
      ...
      phone: p.phone,
      email: p.email,
      // `PatientSummary` de Rust siempre trae estos dos campos (nulos si no
      // hay dato); el mock los omitia y el candidato no cumplia el contrato.
...
export async function mockCall<T>(command: string, args?: Record<string, unknown>): Promise<T> {
  ...
  switch (command) {
    ...
    case "open_encounter":
      // Rust devuelve el `Encounter` completo, no solo su id.
    ...
    case "ai_review_run": {
      ...
      if (!run) throw "ejecucion de IA no encontrada";
      run.status = String(args?.status ?? run.status);
      run.reported = false;
      return mockAiRunDto(run) as T;
    }
    case "ai_list_runs":
      return mockState.aiRuns.map(mockAiRunDto) as T;
    case "ai_set_budget":
    ...
    case "arco_export_patient_data":
      return {
        ...
        // Responsable/tutor y quien ejercio el derecho (pasos 18 y 12): la
        // exportacion ARCO real los incluye siempre.
        guardian: null,
        is_minor: false,
        rights_exercised_by: null,
        encounters: [],
        documents: [],
