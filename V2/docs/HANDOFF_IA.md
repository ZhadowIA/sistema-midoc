...
- **`.env.local` con proveedores reales**: los defaults mock del setupFile solo aplican si la variable está vacía; un `.env.local` con `EMAIL_PROVIDER=resend` o `SMS_PROVIDER=twilio` sí llega a los tests. Las pruebas que dependen del mock (p. ej. el procesamiento de la cola en `notifications.integration.test.ts`) fijan `env.EMAIL_PROVIDER = "mock"` y lo restauran en `finally`, como ya hacen las de Twilio/Resend en sentido inverso. Cualquier test nuevo que "envíe" debe hacer lo mismo.
- **Suite determinista contra base limpia (2026-09-03)**: `npm test` debe pasar contra una base recién migrada (`prisma migrate deploy`, sin `seed`) en cualquier fecha. Reglas: nada de fechas de calendario fijas en tests de disponibilidad (usar helpers relativos al reloj, como `nextFutureCalendarDate` en `public-booking.integration.test.ts`); los usos de IA que se cobran deben ocurrir en el mes UTC en curso, porque `getDoctorAiCreditSummary` solo suma ese mes; y los planes se crean vía `createDoctorSubscription` (upsert), no vía seed.
